...
void adjacencyList();
bool dfsAdjacencyList(Array arrayV[],int num,int arrayTs[],int &index); //(有向图&&无向图)邻接表的DFS
void displayAdjacencyMatrix(int **arrayV,int length);
void adjacencyMatrix();
bool dfsAdjacencyMatrix(int **arrayV,int r,bool arrayFlag[],int num,int arrayTs[],int &index); //(有向图&&无向图)邻接矩阵的DFS
...
bool dfsAdjacencyList(Array arrayV[],int num,int arrayTs[],int &index){
    ...
    if(arrayV[num].flag==false){
        ...
        arrayTs[--index]=num; //递归结束时输出对应的点可以得到逆拓扑排序
    }
}
...
bool dfsAdjacencyMatrix(int **arrayV,int r,bool arrayFlag[],int num,int arrayTs[],int &index){
    ...
    arrayTs[--index]=r; //递归结束时输出对应的点可以得到逆拓扑排序
}
